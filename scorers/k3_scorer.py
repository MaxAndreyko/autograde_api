...
def count_unique_spelling_errors(text):
    ...
    tool = language_tool_python.LanguageTool(
        "en-GB", config={"cacheSize": 1000, "pipelineCaching": True}
    )
    mistakes = tool.check(text_without_name)
    # выбираем только уникальные ошибки
    unique_spelling_mistakes = set(
        tuple(mistake.replacements)
...
def get_changed_sentences(original, corrected):
    ...
    diff = difflib.Differ()
    diff_result = list(diff.compare(original_sents, corrected_sents))

    changed_sentences = [sent for sent in diff_result if sent.startswith("+ ")]
    changed_sentences = [sent[2:] for sent in changed_sentences]
