...
from scorers.k3_scorer import count_unique_spelling_errors, count_punctuation_errors, count_lexical_and_grammatical_errors, get_changed_sentences
...
def evaluate_text(df, answer_col, k1_model, k3_model, keywords):
    ...
    words = text.split()
    if len(words) < 90:
        return {'total':0, 'k1': 0, 'k2': 0, 'k3': 0, 'comments': 'Ответ не соответствует требуемому объёму'}
    elif len(words) > 154:
        words = words[:140]
        text = ' '.join(words)
    ...
    if detect(text) != 'en':
      return {'total': 0, 'k1': 0, 'k2': 0, 'k3': 0, 'comments': 'Текст не соответствует условиям задания'}
    ...
    lexical_and_grammatical_errors = count_lexical_and_grammatical_errors(text, corrected_text) - mistakes_spell_punct

    # подсчитываем ошибки, но тут можно и более лояльные варианты придумать
